  ...
  {!isEmailSent && 
    ...
    <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
      ...
      <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
        ...
        <input onChange={e=> setEmail(e.target.value)} value={email} className='bg-transparent outline-none text-white' 
        placeholder='Email Id' type="email" required/>
      </div>
    ...
    <form onSubmit={onSubmitOTP} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='flex justify-between mb-8' onPaste={handlePaste}>
          {Array(6).fill(0).map((_, index)=>(
            <input type="text" maxLength='1' key={index} required className='w-12 h-12 bg-[#333A56]
            text-white text-center text-xl rounded-md' ref={e=> inputRefs.current[index]=e}
      ...
      <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
        ...
        <input onChange={e=> setNewPassword(e.target.value)} value={newPassword} className='bg-transparent outline-none text-white' 
